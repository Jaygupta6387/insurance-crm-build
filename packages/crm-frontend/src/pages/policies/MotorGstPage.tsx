...
export default function MotorGstPage() {
  ...
  return (
    <div className="flex h-full flex-col">
      <Header title="Motor GST" subtitle="GST on OD and TP per product (and single GST for other LOBs)">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/40">
                  {['LOB', 'Product', 'GST on OD', 'GST on TP', 'GST (single)', ''].map((h) => (
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={6} className="py-16 text-center text-sm text-muted-foreground">No GST rules configured.</td></tr>
                ) : (
                  gstRates.map((g) => (
                    <tr key={g.id} className="border-b border-border/40 hover:bg-muted/30">
                      <td className="px-4 py-3 text-foreground">{g.lob?.name || <span className="text-muted-foreground">All</span>}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => { setEditing(g); setModalOpen(true); }} className="rounded-md p-1.5 text-muted-foreground hover:bg-muted"><Pencil className="h-3.5 w-3.5" /></button>
                          <button onClick={() => remove(g.id)} className="rounded-md p-1.5 text-muted-foreground hover:bg-muted hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                        </div>
                      </td>
                    </tr>
                  ))
...
function GstModal({ gst, lobs, products, onClose, onSaved }: {
  gst: GstRate | null;
  ...
  const [form, setForm] = useState<GstRatePayload>({
    ...
    gst_percent: gst?.gst_percent != null ? Number(gst.gst_percent) : null,
  ...
  const save = async () => {
    setSaving(true);
    try {
      const payload = { ...form, lob_id: form.lob_id || null, product_id: form.product_id || null };
      ...
      onSaved();
    } catch { toast.error('Failed to save GST rule'); } finally { setSaving(false); }
  };
    ...
    <div className="fixed inset-0 z-[9990] flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-card shadow-2xl">
        ...
        <div className="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2">
          <div>
            <label className="text-xs font-semibold text-foreground/80">LOB (optional)</label>
            <select value={form.lob_id || ''} onChange={(e) => set({ lob_id: e.target.value })} className={cn(inputCls, 'mt-1.5')}>
          ...
          <div className="sm:col-span-2">
            <label className="text-xs font-semibold text-foreground/80">Single GST % (non-motor LOBs)</label>
            <input type="number" step="0.01" value={form.gst_percent ?? ''} onChange={(e) => set({ gst_percent: e.target.value === '' ? null : Number(e.target.value) })} placeholder="Leave blank for motor" className={cn(inputCls, 'mt-1.5')} />
          </div>
          <div className="sm:col-span-2">
            <label className="flex items-center gap-2 text-sm text-foreground">
              ...
              Apply to all products of the selected LOB
            </label>
          </div>
        </div>
